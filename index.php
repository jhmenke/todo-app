        ...
        tailwind.config = {
            theme: {
                extend: {
                    ...
                    colors: { accent: { DEFAULT: '#4f46e5', soft: '#eef2ff', dark: '#4338ca' } }
                }
            }
        }
    </script>
...
<div class="flex flex-1 overflow-hidden">
        ...
        <div class="p-3 pb-8">
            ...
            <template x-for="tag in tags" :key="tag.id">
                <div class="sidebar-item group" :class="filterTagId===tag.id?'active':''" @click="setTag(tag.id)">
                    ...
                    <button @click.stop="deleteTag(tag.id)" class="opacity-0 group-hover:opacity-100 text-slate-400 hover:text-red-500 transition-opacity">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                    </button>
                </div>
            </template>
            <div x-show="tags.length===0" class="px-4 py-1 text-xs text-slate-400" x-text="t('nav.no_tags')"></div>
        </div>
    </aside>
    ...
    <main class="flex-1 overflow-y-auto">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 py-7">
            ...
            <div x-show="filterStatus==='all'" class="flex items-center gap-2 mb-4">
                <label class="flex items-center gap-2 cursor-pointer select-none text-sm text-slate-600">
                    <input type="checkbox" x-model="hideCompleted" @change="loadTodos()"
                        class="w-4 h-4 rounded border-slate-300 accent-[#4f46e5]">
            ...
            <!-- Loading -->
            <div x-show="loading" class="text-center py-16 text-slate-400 text-sm" x-text="t('list.loading')"></div>
            ...
            <div x-show="!loading && todos.length===0" class="empty-state">
                ...
                <p class="text-slate-700 font-semibold" x-text="searchQuery ? t('list.no_matches') : t('list.empty_title')"></p>
                <p class="text-slate-400 text-sm mt-1" x-text="searchQuery ? t('list.no_matches_body') : t('list.empty_body')"></p>
                    ...
                    <div class="todo-card group"
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex items-center gap-3 mt-1.5 text-xs text-slate-400 flex-wrap">
                                ...
                                <span x-show="!todo.active_at && !todo.completed_at" class="text-slate-300" x-text="t('list.no_due')"></span>
                                <span x-show="todo.recur_type" class="text-emerald-600 font-medium" x-text="recurLabel(todo)"></span>
                                <span x-show="todo.children && todo.children.length" class="text-slate-400"
                    ...
                    <template x-for="child in (todo.children || [])" :key="'child-'+child.id">
                        <div class="todo-card todo-child group"
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="flex items-center gap-3 mt-1 text-xs text-slate-400" x-show="child.active_at">
...
<div x-show="drawer" x-transition:enter="opacity-0" x-transition:enter-end="opacity-100"
     class="fixed inset-0 bg-slate-900/40 z-30 backdrop-blur-sm transition-opacity duration-200"
        ...
        <div class="flex flex-col h-full">
            ...
            <div class="flex items-start gap-3 px-5 py-5 border-b" style="border-color:var(--line)">
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs text-slate-400 mt-2" x-show="drawer.active_at && !drawer.completed_at">
                        <span x-text="t('task.activates')"></span> <span x-text="formatDateShort(drawer.active_at)" class="font-medium text-slate-600"></span>
                    </p>
                    ...
                    <p class="text-xs text-slate-400 mt-2" x-show="drawer.parent_title">
                        <span x-text="t('task.subtask_of')"></span> <span class="font-medium text-slate-600" x-text="drawer.parent_title"></span>
                    </p>
                </div>
            ...
            <div class="px-5 py-3" style="border-bottom:1px solid var(--line)" x-show="!drawer.parent_id && drawer.is_owner">
                <p class="text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2" x-text="t('task.subtasks')"></p>
                    ...
                    <template x-for="child in (drawer.children || [])" :key="child.id">
                        <div class="flex items-center gap-2 py-1">
                            ...
                            <button type="button" class="text-sm text-left flex-1 min-w-0 truncate"
                                :class="child.completed_at ? 'line-through text-slate-400' : ''"
                                x-text="child.title"
                                @click="openDrawer(child)"></button>
                        </div>
                    </template>
                    <div x-show="!(drawer.children && drawer.children.length)" class="text-xs text-slate-400" x-text="t('task.none_yet')"></div>
                </div>
            ...
            <div x-show="drawerTab==='comments'" class="flex-1 flex flex-col overflow-hidden">
                <div class="flex-1 overflow-y-auto px-5 py-4 space-y-4" x-show="!drawerLoading">
                    <div x-show="comments.length===0" class="text-center py-10 text-sm text-slate-400" x-text="t('comments.empty')"></div>
                        ...
                        <div class="flex gap-3">
                            ...
                            <div class="flex-1">
                                <div class="flex items-baseline gap-2">
                                    ...
                                    <span class="text-xs text-slate-400" x-text="formatDateShort(c.created_at)"></span>
                                </div>
                                <p class="text-sm text-slate-700 mt-0.5 whitespace-pre-wrap" x-text="c.body"></p>
                            </div>
                        </div>
                    </template>
                </div>
                <div x-show="drawerLoading" class="flex-1 flex items-center justify-center text-sm text-slate-400" x-text="t('list.loading')"></div>
                ...
                <div class="px-5 py-3" style="border-top:1px solid var(--line)">
                    ...
                    <p class="text-xs text-slate-400 mt-1" x-text="t('comments.hint')"></p>
                </div>
            </div>
            ...
            <div x-show="drawerTab==='files'" class="flex-1 flex flex-col overflow-hidden">
                <div class="flex-1 overflow-y-auto px-5 py-4 space-y-2">
                    <div x-show="files.length===0 && !drawerLoading" class="text-center py-10 text-sm text-slate-400" x-text="t('files.empty')"></div>
                    <template x-for="f in files" :key="f.id">
                        <div class="file-row group">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs text-slate-400" x-text="formatBytes(f.size_bytes) + ' · ' + (f.uploader_email || t('files.you'))"></p>
                            </div>
                ...
                <div class="px-5 py-3" style="border-top:1px solid var(--line)">
                    ...
                    <label class="inline-flex items-center gap-3 cursor-pointer">
                        ...
                        <span class="text-xs text-slate-400" x-text="t('files.limits')"></span>
                    </label>
                </div>
            </div>
            ...
            <div x-show="drawer.is_owner && drawerTab==='shares'" class="flex-1 flex flex-col overflow-hidden">
                <div class="flex-1 overflow-y-auto px-5 py-4 space-y-2">
                    ...
                    <div x-show="shares.length===0" class="text-sm text-slate-400 py-2" x-text="t('share.empty')"></div>
                </div>
                <div class="px-5 py-3" style="border-top:1px solid var(--line)">
                    ...
                    <p class="text-xs text-slate-400 mt-1" x-text="t('share.hint')"></p>
                </div>
            </div>
        </div>
    </template>
...
<div class="modal-bg" @keydown.escape.window="closeModal()" @keydown.ctrl.s.window.prevent="saveForm()">
    <div class="modal-panel max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div class="px-6 py-5 space-y-5">
            ...
            <!-- Activate at -->
            <div>
                <label class="block text-sm font-medium mb-1"><span x-text="t('form.activate')"></span> <span class="text-slate-400 font-normal" x-text="t('form.optional')"></span></label>
                <div class="flex gap-2">
            ...
            <div x-show="!form.parent_id">
                ...
                <div x-show="form.recur_type==='weekly'" class="mt-3">
                    <p class="text-xs text-slate-500 mb-2" x-text="t('form.repeat_on')"></p>
                    <div class="flex gap-1.5 flex-wrap">
                        <template x-for="d in days()" :key="d">
                            <button type="button" @click="toggleFormDay(d)"
                                class="w-9 h-9 rounded-full text-xs font-semibold transition-colors"
                                :class="form.recur_days.includes(d) ? 'bg-accent text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'"
                ...
                <div x-show="form.recur_type==='custom'" class="mt-3 flex items-center gap-2">
                    <span class="text-sm text-slate-600" x-text="t('form.every')"></span>
                    <input type="number" x-model="form.recur_interval" min="1" max="365" class="w-20 text-center">
                    <span class="text-sm text-slate-600" x-text="t('form.days')"></span>
                </div>

                <!-- Ends at -->
                <div x-show="form.recur_type" class="mt-3">
                    <label class="block text-xs text-slate-500 mb-1"><span x-text="t('form.ends_at')"></span> <span class="text-slate-400" x-text="t('form.optional')"></span></label>
    ...
    <div class="modal-panel max-w-md mx-4">
        ...
        <div class="px-6 py-5 space-y-6">
            <div>
                ...
                <div class="flex gap-2">
                    <label class="flex items-center gap-2 cursor-pointer text-sm">
                        <input type="radio" x-model="settingsLocale" value="en" class="accent-[#4f46e5]">
                        <span x-text="t('settings.english')"></span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer text-sm">
                        <input type="radio" x-model="settingsLocale" value="de" class="accent-[#4f46e5]">
            ...
            <div>
                ...
                <div class="flex flex-col gap-1.5 text-sm">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" x-model="settingsChannel" value="telegram" class="accent-[#4f46e5]">
                        <span x-text="t('settings.telegram')"></span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" x-model="settingsChannel" value="email" class="accent-[#4f46e5]">
                        <span x-text="t('settings.email')"></span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" x-model="settingsChannel" value="both" class="accent-[#4f46e5]">
            ...
            <div x-show="settingsChannel === 'telegram' || settingsChannel === 'both'">
                ...
                <p x-show="!telegramConfigured" class="text-xs text-slate-400 leading-relaxed" x-text="t('settings.telegram_off')"></p>
            ...
            <div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-sm text-slate-600" x-text="t('settings.minutes_before')"></span>
                </div>
